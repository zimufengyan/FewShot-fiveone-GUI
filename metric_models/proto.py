...
class ProtoModel(MetricModelBase):
    ...
    def __init__(self, input_shape, out_channels, num_hiddens=64, lr=0.001, gpu_ids=[0],
                 distance='euclidean', is_train=True, lr_policy='step', **kwargs) -> None:
        super().__init__(gpu_ids, distance=distance, is_train=is_train, 
                         lr_policy=lr_policy, **kwargs)
        
        self.embedding_net = EmbeddingNet(input_shape, out_channels, num_hiddens, num_layers=4)
        self.nets = ['embedding']
