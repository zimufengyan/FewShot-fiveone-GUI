...
def init_lstm_hiddens(shape, init_type, init_gain=0.02) -> tuple:
    h0 = torch.zeros(size=shape, names='h')
    c0 = torch.zeros(size=shape, names='c')
    if init_type == 'normal':
        nn.init.normal_(h0, 0.0, init_gain)
        nn.init.normal_(c0, 0.0, init_gain)
    elif init_type == 'xavier':
        nn.init.xavier_normal_(h0, 0.0, init_gain)
        nn.init.xavier_normal_(c0, 0.0, init_gain)
    elif init_type == 'kaiming':
        nn.init.kaiming_normal_(h0, 0.0, init_gain)
        nn.init.kaiming_normal_(c0, 0.0, init_gain)
    elif init_type == 'orthogonal':
        nn.init.orthogonal_(h0, 0.0, init_gain)
        nn.init.orthogonal_(c0, 0.0, init_gain)
    else:
        raise NotImplementedError('initialization method [%s] is not implemented' % init_type)
