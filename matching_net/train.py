...
if __name__ == '__main__':
    ...
    model = MatchingModel(
        in_channels, opt.out_channels, opt.n_hiddens, opt.learning_rate, gpu_ids, is_train=True, input_shape=input_shape,
        lr_policy=opt.lr_scheduler_policy, gamma=opt.lr_gamma, lr_decay_iters=opt.lr_decay_epochs, distance=opt.distance_fn,
        use_fce=bool(opt.use_fce)
    )
    ...
    save_name = f'matchingnet_on_{opt.dataset_name}_at_{opt.n_epochs+1}'
    model.save_networks(save_dir=opt.save_dir, name=save_name)
